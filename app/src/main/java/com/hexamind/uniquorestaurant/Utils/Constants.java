package com.hexamind.uniquorestaurant.Utils;

public class Constants {
    public static final String SHARED_PREFS_NAME = "SharedPrefs";
    public static final String CUSTOMER_OBJ_NAME = "CustomerObjName";
    public static final String FOOD_ITEM_STRING = "FoodItem";
    public static final String FOOD_ITEM_MAP_STRING = "FoodItemMap";
    public static final String IS_TABLE_BOOKED = "isTableBooked";
    public static final String TABLE_ID_CONST_STRING = "tableId";
    public static final String TABLE_EXISTS_ALREADY_STRING = "tableAlreadyExists";
    public static final String TABLE_EXISTS_MAP_ALREADY_STRING = "tableAlreadyExistsMap";
    public static final String USER_ORDERS_STRING = "UserOrders";
    public static final String FOOD_ITEM_ID = "FoodItemId";
    public static final String MANAGER_OBJ_NAME = "ManagerObjName";
    public static final String TABLE_ID_MAP_CONST_STRING = "tableIdMap";
    public static String CUSTOMER_STRING = "customer";
}
